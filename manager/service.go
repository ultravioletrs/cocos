...
func (ms *managerService) Run(ctx context.Context, c *manager.ComputationRunReq) (string, error) {
	ms.publishEvent(VmProvision.String(), c.Id, Starting.String(), json.RawMessage{})
	...
	for _, data := range c.Datasets {
		if len(data.Hash) != hashLength {
			ms.publishEvent(VmProvision.String(), c.Id, agent.Failed.String(), json.RawMessage{})
	...
	ms.publishEvent(VmProvision.String(), c.Id, agent.InProgress.String(), json.RawMessage{})
	...
	ms.publishEvent(VmProvision.String(), c.Id, agent.Completed.String(), json.RawMessage{})
...
func (ms *managerService) Stop(ctx context.Context, computationID string) error {
	...
	if !ok {
		defer ms.publishEvent(StopComputation.String(), computationID, agent.Failed.String(), json.RawMessage{})
		return ErrNotFound
	}
	...
	defer ms.publishEvent(StopComputation.String(), computationID, agent.Completed.String(), json.RawMessage{})
	return nil
}
